...
const char * const BITCOIN_CONF_FILENAME = "educacoin.conf";
const char * const BITCOIN_PID_FILENAME = "educacoind.pid";
...
#else
    const char* pszModule = "educacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Educacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Educacoin";
#else
    // Unix
    return pathRet / ".educacoin";
#endif
#endif
}
