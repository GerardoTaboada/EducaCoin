...
LITECOIND=${LITECOIND:-$SRCDIR/educacoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/educacoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/educacoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/educacoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
EDCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
